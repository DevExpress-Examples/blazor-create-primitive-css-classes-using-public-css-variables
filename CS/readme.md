...
This example demonstrates two different ways to build simple layouts using primitive blocks. The application includes two pages that display an employee card:
...
The Bootstrap approach allows you to build the UI using predefined classes without additional CSS, but its spacing, size, and color styles scalability is fixed (you cannot use arbitrary or intermediate values). Dynamic styling scenarios may require additional classes or custom CSS/inline styles. 
